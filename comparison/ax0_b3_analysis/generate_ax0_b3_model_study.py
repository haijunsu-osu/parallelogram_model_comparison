...
REPORT_PATH = SCRIPT_DIR / "comparison_accuracy_Ax_0_B_3.md"
AX_TARGET = 0.0
B_TARGET = 3.0
AY_MIN = 0.0
AY_MAX = 20.0
...
def is_pct_error_row(row):
    return not math.isclose(float(row["Ay"]), 0.0, rel_tol=0.0, abs_tol=1e-12)
...
def plot_series(rows, metric, ylabel, output_name):
    ...
    ax.set_title(
        rf"{ylabel} vs. $\alpha_y$ for all models $(\alpha_x = 0,\ \beta = 3)$",
        fontsize=13,
    )
...
def plot_error(rows, metric, ylabel, output_name, pct=False):
    fig, ax = plt.subplots(figsize=(8.8, 5.8))
    plot_rows = [row for row in rows if is_pct_error_row(row)] if pct else rows
    ...
    ax.set_title(
        rf"{title_label.title()} vs. $\alpha_y$ relative to FEA 3D $(\alpha_x = 0,\ \beta = 3)$",
        fontsize=13,
    )
    ax.grid(True, linestyle=":", alpha=0.5)
...
def build_report(rows):
    ay_values = [float(row["Ay"]) for row in rows]
    pct_ay_values = [float(row["Ay"]) for row in rows if is_pct_error_row(row)]
    lines = []
    lines.append("# Accuracy Comparison For `Ax = 0`, `B = 3`")
    lines.append("")
    lines.append(
        "This study compares all models in the `Ax = 0`, `B = 3`, `Ay in [0, 20]` slice "
    ...
    lines.append("## Method")
    lines.append("")
    lines.append("- Slice used: `Ax = 0`, `B = 3`, `0 <= Ay <= 20`.")
    ...
    lines.append("- Percentage-error summaries and percentage-error plots exclude the `Ay = 0` point.")
    lines.append("")
    ...
    lines.append("")
    lines.append("`" + ", ".join(f"{value:g}" for value in pct_ay_values) + "`")
    lines.append("")
    ...
    lines.append("")
    for metric, _ylabel, curve_name, _abs_err_label, abs_err_name, _pct_label, pct_name in METRIC_SPECS:
